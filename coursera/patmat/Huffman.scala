...
object Huffman {
   ...
   */
   def times(chars: List[Char]): List[(Char, Int)] = {
      def aux(c: Char, cList: List[(Char, Int)]): List[(Char, Int)] = {
        if (cList.isEmpty) (c, 1) :: Nil
        else {
          val found = cList.filter(x => x._1 == c)
          if (found == Nil) (c, 1) :: cList
          else (c, found.head._2 + 1) :: cList.filter(x => x._1 != c)
        }
      }
      def recur(cs: List[Char], cList: List[(Char, Int)]): List[(Char, Int)] = {
        if (cs.isEmpty) cList
        else recur(cs.tail, aux(cs.head, cList))
      }
      recur(chars, Nil)
   }
   ...
   */  
   def combine(trees: List[CodeTree]): List[CodeTree] = {
     if(trees.length < 2) trees
       ...
       (makeCodeTree(frst,scnd)::trees.tail.tail).sortWith((x,y) => weight(x) < weight(y))
     }
   }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAux(subtree: CodeTree, bits: List[Bit], accu: List[Char]): List[Char] = bits match {
	    case Nil => subtree match {
	      case l: Leaf => accu :+ l.char
	      case f: Fork => accu
 	    }
	    case x :: xs => subtree match {
	      case l: Leaf => decodeAux(tree, bits, accu :+ l.char)
	      case f: Fork => decodeAux(if (x == 0) f.left else f.right, xs, accu)
	    }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    //rec call to encode all the chars
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def code(text: List[Char], bits: List[Bit]): List[Bit] = {
      if (text.isEmpty) bits
      else codeBits(codeTable)(text.head) ::: code(text.tail, bits)
    }
    code(text, Nil)
  }
